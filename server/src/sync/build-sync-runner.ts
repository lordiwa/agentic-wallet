 ...
 * requires a titular to reconcile internal transfers (F1-04), so syncing
 * without one would just produce silently-wrong `is_internal` flags.
 */
 ...
 */
export function buildProductionSyncRunner(config: Config, getDb: () => Database.Database): SyncRunner | null {
